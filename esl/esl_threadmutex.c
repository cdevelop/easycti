...
{
	void *exit_val;
    esl_thread_t *_thread = (esl_thread_t *)args;
	exit_val = _thread->function(_thread, _thread->private_data);
#ifndef WIN32
	pthread_attr_destroy(&_thread->attribute);
#endif
	free(_thread);
...
{
	esl_thread_t *_thread = NULL;
	esl_status_t status = ESL_FAIL;

	if (!func || !(_thread = (esl_thread_t *)malloc(sizeof(esl_thread_t)))) {
		goto done;
	}

	_thread->private_data = data;
	_thread->function = func;
	_thread->stack_size = stack_size;

#if defined(WIN32)
	_thread->handle = (void *)_beginthreadex(NULL, (unsigned)_thread->stack_size, (unsigned int (__stdcall *)(void *))thread_launch, _thread, 0, NULL);
	if (!_thread->handle) {
		goto fail;
	}
	...
	if (pthread_attr_init(&_thread->attribute) != 0)	goto fail;

	if (pthread_attr_setdetachstate(&_thread->attribute, PTHREAD_CREATE_DETACHED) != 0) goto failpthread;

	if (_thread->stack_size && pthread_attr_setstacksize(&_thread->attribute, _thread->stack_size) != 0) goto failpthread;

	if (pthread_create(&_thread->handle, &_thread->attribute, thread_launch, _thread) != 0) goto failpthread;
